...
void *cv_capture_from_camera(int cam_index, int w, int h, int frames) {
    ...
    if (w) {
        cvSetCaptureProperty((CvCapture *) cap, CV_CAP_PROP_FRAME_WIDTH, w);
    }
    if (h) {
        cvSetCaptureProperty((CvCapture *) cap, CV_CAP_PROP_FRAME_HEIGHT, h);
    }
    if (frames) {
        cvSetCaptureProperty((CvCapture *) cap, CV_CAP_PROP_FPS, frames);
    }
